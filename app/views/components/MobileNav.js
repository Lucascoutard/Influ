  ...
  render() {
    ...
    return `
      <div class="mobile-nav" id="mobileNav">
        <button class="mobile-close" id="mobileClose" aria-label="Fermer le menu">&times;</button>
        ${allLinks.map(l => `
          <a href="${l.href}" class="mobile-nav-link" onclick="NavigationController.closeMobile()">${l.label}</a>
        `).join('')}
        ${isLoggedIn
          ? `<a href="#logout" class="btn-connect" onclick="NavigationController.closeMobile()">Déconnexion</a>`
